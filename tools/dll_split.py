...
class DLLSplitter:
    ...
    def split_dlls(self, only_dlls: "list[str]"):
        # Load DLLS.tab
        if self.verbose:
        ...
        for number in range(796):
            ...
            self.__split_dll_common(tab, core_export_syms, number, dir, dll_dir)
    ...
    def split_dll(self, 
                  ...
                  default_c_filename: str | None=None):
        ...
        self.__split_dll_common(tab, core_export_syms, number, dir, dll_dir, prefix, default_c_filename)

    def __split_dll_common(self, 
                           tab: DLLTab,
                           core_export_syms: DLLSymsTxt,
                           number: int,
                           dir: Path | None,
                           dll_dir: str | None,
                           prefix: str | None=None,
        ...
        # Default prefix
        if prefix == None:
            prefix = f"dll_{number}"
            type_prefix = f"DLL{number}"
        else:
            # Ensure the prefix doesn't end with an underscore, we'll add that later
        ...
        with open(dll_path, "rb") as dll_file:
            ...
            if dll_syms == None:
                self.__default_symbol_names(analyzed_dll, prefix)
            ...
            if dll_dir != None:
                self.__extract_dll(analyzed_dll, dll_dir, type_prefix, default_c_filename)
    ...
    def __default_symbol_names(self, dll: AnalyzedDLL, prefix: str):
        ...
        for sym in dll.context.globalSegment.symbols.values():
            if sym.sectionType == spimdisasm.common.FileSectionType.Text and \
               sym.type == spimdisasm.common.SymbolSpecialType.function:
                # sym.vram >= dll.text.vram excludes imports. DLL analysis sets up addresses as .text >= 0x81000000 > imports
                if sym.vram >= dll.text.vram and sym.name != None and sym.name.startswith("dll_"):
                    addr = sym.vram - dll.text.vram
                    sym.name = f"{prefix}_func_{addr:X}"
        
        # ctor
        ctor_sym = dll.context.globalSegment.addSymbol(DLL_VRAM_BASE + dll.meta.header.ctor_offset,
                                                       sectionType=spimdisasm.common.FileSectionType.Text,
                                                       isAutogenerated=True)
        ctor_sym.name = f"{prefix}_ctor"
        # dtor
        dtor_sym = dll.context.globalSegment.addSymbol(DLL_VRAM_BASE + dll.meta.header.dtor_offset,
                                                       sectionType=spimdisasm.common.FileSectionType.Text,
                                                       isAutogenerated=True)
        dtor_sym.name = f"{prefix}_dtor"
    ...
    def __extract_dll(self, 
        ...
        if nonmatching_funcs == None:
            if default_c_filename == None:
                default_c_filename = f"{dll.number}.c"
            else:
                # Force .c suffix
                default_c_filename = Path(default_c_filename).with_suffix(".c").as_posix()
            c_file_path = src_path.joinpath(default_c_filename)
            self.__create_c_stub(c_file_path, nonmatching_asm_path, dll, type_prefix)
    ...
    def __create_c_stub(self, 
        ...
        # If an object DLL, try to figure out the object data size for auto decomp
        obj_data_size: int | None = None
        obj_data_name: str | None = None
        if dll.number >= 210:
            obj_data_size = self.__try_find_object_data_size(dll, exports)
            obj_data_name = f"{type_prefix}_Data"
        ...
        with open(c_path, "w", encoding="utf-8") as c_file:
            ...
            if dll.number >= 210:
                if obj_data_size != None and obj_data_size != 0:
                    c_file.write("\ntypedef struct {\n")
                    c_file.write(f"    u8 _unk0[0x{obj_data_size:X}];\n")
                    c_file.write(f"}} {obj_data_name};\n")
            ...
            for func_rodata in dll.function_rodata:
                if func_rodata.function == None:
                    ...
                    if not self.__try_auto_decomp(dll, func, ctor_dtor, export_idx, obj_data_size, obj_data_name, c_file):
    ...
    DEFAULT_OBJ_PRINT = [0xAFA40000, 0xAFA50004,  0xAFA60008, 0xAFA7000C, 0x03E00008, 0x000000]
    DEFAULT_OBJ_FREE = [0xAFA40000, 0xAFA50004, 0x03E00008, 0x000000]
    ...
    def __try_auto_decomp(self, 
                          ...
                          ctor_dtor: bool,
                          export_idx: int | None,
                          obj_data_size: int | None,
                          obj_data_name: str | None,
                          c_file: TextIO) -> bool:
        if ctor_dtor:
        ...
        if dll.number >= 210:
            ...
            if export_idx == 0:
                ...
                if self.__instructions_equal(func, self.DEFAULT_OBJ_PRINT):
                ...
                if obj_data_size != None:
                    ...
                    if obj_data_size == 0:
                        ...
                        c_file.write(f'    return sizeof({obj_data_name});\n')
                    c_file.write('}\n')
                    return True
                else:
                    c_file.write(f'u32 {func.getName()}(Object* self, u32 offsetAddr);\n')
    ...
    def __try_find_object_data_size(self, dll: AnalyzedDLL, exports: dict[int, int]):
        if dll.number < 210:
            return None
...
def main():
    ...
    split_parser.add_argument("dlls", nargs="*", action="extend", help="The numbers of each DLL to extract. Don't specify any to extract all that have src directories.")

    split_single_parser = subparsers.add_parser("split-single")
    split_single_parser.add_argument("dll", help="The number of the DLL to extract.")
    split_single_parser.add_argument("--prefix", type=str, help="The default function name prefix to use, if setting up for the first time.")
    split_single_parser.add_argument("--filename", type=str, help="The default filename to use, if setting up for the first time.")
    ...
    if args.cmd == "split":
        splitter.split_dlls(args.dlls)
    elif args.cmd == "split-single":
        splitter.split_dll(args.dll, args.prefix, args.filename)
    else:
        print(f"ERR: Unknown command: {args.cmd}")
        exit(1)
