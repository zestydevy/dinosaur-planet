...
from .dll import DLL
from .dll_syms_txt import DLLSymsTxt, DLLSym, DLLSymSection
        ...
        if sym.vram >= DLL_VRAM_BASE:
            if bss != None and sym.vram >= bss.vram and sym.vram < bss.vramEnd:
                global_segment.addSymbol(sym.vram, spimdisasm.common.FileSectionType.Bss,
                                         isAutogenerated=sym.isAutogenerated)
                ...
                if sym.getTypeSpecial() == spimdisasm.common.SymbolSpecialType.function:
                    global_segment.addFunction(sym.vram, isAutogenerated=sym.isAutogenerated)
                else:
...
def __handle_dll_sym_attributes(
        address: int,
        dll_sym: DLLSym,
        dll_section: DLLSymSection | None, 
        section: spimdisasm.mips.sections.SectionBase | spimdisasm.common.SymbolsSegment):
    attrs = dll_sym.attributes
    
    if dll_section != None:
        address += DLL_VRAM_BASE + dll_section.offset

    type = attrs.get("type")
    if type == "func":
        contextSym = section.addFunction(address, isAutogenerated=False,)
    elif type == "jtbl":
        contextSym = section.addJumpTable(address, isAutogenerated=False)
    elif type == "jtbl_label":
        contextSym = section.addJumpTableLabel(address, isAutogenerated=False)
    elif type == "label":
        contextSym = section.addBranchLabel(address, isAutogenerated=False)
    else:
        if isinstance(section, spimdisasm.mips.sections.SectionBase):
            contextSym = section.addSymbol(address, isAutogenerated=False,
    ...
    contextSym.name = dll_sym.name
    contextSym.isUserDeclared = True
    contextSym.nameEnd = attrs.get("name_end")
    contextSym.userDeclaredSize = spimdisasm.common.Utils.getMaybeIntFromMaybeStr(attrs.get("size"))

    defined = spimdisasm.common.Utils.getMaybeBooleyFromMaybeStr(attrs.get("defined"))
    if defined is not None:
        contextSym.isDefined = defined

    forceMigration = spimdisasm.common.Utils.getMaybeBooleyFromMaybeStr(attrs.get("force_migration"))
    if forceMigration is not None:
        contextSym.forceMigration = forceMigration
    forceNotMigration = spimdisasm.common.Utils.getMaybeBooleyFromMaybeStr(attrs.get("force_not_migration"))
    if forceNotMigration is not None:
        contextSym.forceNotMigration = forceNotMigration
    contextSym.functionOwnerForMigration = attrs.get("function_owner")

    contextSym.visibility = attrs.get("visibility")
...
def __define_known_symbols(symbols_file: DLLSymsTxt,
    ...
    for address, sym in symbols_file.absolute.items():
        __handle_dll_sym_attributes(address, sym, None, global_segment)

    text_symbols = symbols_file.sections.get(".text")
    if text_symbols != None:
        for address, sym in text_symbols.syms.items():
            sym.attributes.setdefault("type", "func")
            __handle_dll_sym_attributes(address, sym, text_symbols, text)

    if rodata != None:
        rodata_symbols = symbols_file.sections.get(".rodata")
        if rodata_symbols != None:
            for address, sym in rodata_symbols.syms.items():
                __handle_dll_sym_attributes(address, sym, rodata_symbols, rodata)
    
    if data != None:
        data_symbols = symbols_file.sections.get(".data")
        if data_symbols != None:
            for address, sym in data_symbols.syms.items():
                __handle_dll_sym_attributes(address, sym, data_symbols, data)
    
    if bss != None:
        bss_symbols = symbols_file.sections.get(".bss")
        if bss_symbols != None:
            for address, sym in bss_symbols.syms.items():
                __handle_dll_sym_attributes(address, sym, bss_symbols, bss)
...
def __define_symbols_from_reloc_table(
    ...
    for gp_reloc in dll.reloc_table.gp_relocations:
        text.addFunction(text.vram + gp_reloc)
