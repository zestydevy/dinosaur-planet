...
void dl_add_debug_info(Gfx *gdl, u32 objID, const char *file, u32 line) {
    if (gDLDebugInfos[gDLDebugInfoIdx] != NULL && gDLDebugInfoLengths[gDLDebugInfoIdx] < MAX_DL_DEBUG_INFO_LENGTH)
    {
        ...
        gDLDebugInfos[gDLDebugInfoIdx][gDLDebugInfoLengths[gDLDebugInfoIdx]].line = line;
    ...
    *file = NULL;
    for (i = 0; i < gDLDebugInfoLengths[gDLDebugInfoIdx]; i++) {
        ...
        if (gdl == curr) {
            ...
            *arg2 = gDLDebugInfos[gDLDebugInfoIdx][i].line;
            *arg3 = gDLDebugInfos[gDLDebugInfoIdx][i].objID;
            break;
        }

        if (gdl == prev) {
            *file = gDLDebugInfos[1 - gDLDebugInfoIdx][i].file;
            *arg2 = gDLDebugInfos[1 - gDLDebugInfoIdx][i].line;
            *arg3 = gDLDebugInfos[1 - gDLDebugInfoIdx][i].objID;
            break;
        }
