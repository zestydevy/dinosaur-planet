...
s32 dll_1_func_F5C(Object **arg0, s32 arg1, u8 arg2, s32 arg3, f32 distance) {
    s32 _pad[2];
    Vec3f delta;
    Camera* camera;
    f32 childX; //sp 9C
    ...
    for (i = start; i < count; i++) {
        obj = objects[i];
        if ((obj->def->unk40 != NULL) && (obj->opacity == OBJECT_OPACITY_MAX) && !(obj->unkAF & 8) && 
                (obj->def->unk40->unk10 & arg2) && (matchCount < arg1) && (arg3 & 1)) {
            get_object_child_position(obj, &childX, &childY, &childZ);
            delta.x = childX - camera->srt.transl.x;
            delta.y = childY - camera->srt.transl.y;
            delta.z = childZ - camera->srt.transl.z;
            if ((SQ(delta.x) + SQ(delta.y) + SQ(delta.z)) < SQ(distance)) {
                dYaw = camera->srt.yaw - ((0x4000 - arctan2_f(delta.x, delta.z)) & 0xFFFF);
                CIRCLE_WRAP(dYaw);
                if ((dYaw < -0x2710) && (dYaw > -0x55F0)) {
                    arg0[matchCount] = obj;
                    matchCount += 1;
                }
            }
    ...
    if (matchCount > 0) {
        ...
        // for (var_a3 = 1, i = 0; var_a3 && i < (matchCount - 1); i++){
        //     temp_v1 = arg0[i];
        //     temp_a0 = arg0[i + 1];
        //     if ((s32)temp_v1 < (s32)temp_a0) {
        //         arg0[i] = temp_a0;
