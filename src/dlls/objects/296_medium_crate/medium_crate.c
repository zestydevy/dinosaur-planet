...
void medium_crate_setup(Object *self, MediumCrate_Setup *setup, s32 param3) {
    ...
    _data_0 = dllLoadActual(DLL_ID_107, 1, /*bRunConstructor=*/FALSE);
    _data_4 = dllLoadActual(DLL_ID_106, 1, /*bRunConstructor=*/FALSE);

    objdata->unk8 = 0;
    objdata->unkE = mathRnd(0, 100) + 300;
    objdata->unk10 = 400;
...
void medium_crate_control(Object *self) {
    ...
    if (objdata->unk4 > 0.0f) {
        ...
        if (temp_v0_4 != 0) {
            objdata->unk17 += sp78;
            if (objdata->unk17 < 2) {
                dll_amSfx->Play(self, objdata->soundID1, MAX_VOLUME, NULL, NULL, 0, NULL);
                objdata->unk20 = 1.0f;
                objdata->unk24 = 12.0f;
                if (self->id == OBJ_MediumBasket) {
                    objdata->unk1C = mathRnd(600, 800);
                }
                if (temp_v0_4 != 0xF) {
        ...
        temp_fv0_2 = vec3DistanceSquared(&objGetPlayer()->globalPosition, &self->globalPosition);
        objdata->unkE -= gUpdateRate;
        if (objdata->unkE <= 0) {
            objdata->unkE = mathRnd(0, 100) + 300;
            if (temp_fv0_2 < 8100.0f) {
                if ((objdata->unk15 == 5) || (objdata->unk15 == 6) || (objdata->unk15 == 7) || (objdata->unk15 == 8)) {
                    dll_amSfx->Play(NULL, SOUND_64D_Wooden_Rattle, 0x39, NULL, NULL, 0, NULL);
                } else if (objdata->unk15 == 0) {
                    dll_amSfx->Play(NULL, SOUND_6B7_Crate_Hiss, 0x39, NULL, NULL, 0, NULL);
                }
            }
        } else {
            if ((objdata->unk15 == 1) || (objdata->unk15 == 2) || (objdata->unk15 == 3) || (objdata->unk15 == 4)) {
                if (objdata->unk8 == 0) {
                    if (temp_fv0_2 < 8100.0f) {
                        dll_amSfx->Play(self, SOUND_64C_Wicker_Rattle, 0x39, &objdata->unk8, NULL, 0, NULL);
                    }
...
s32 medium_crate_func_C50(Object *self, Object *player, MediumCrate_Data *objdata) {
    ...
    while (spawnsLeft != 0) {
        ...
        switch (objdata->unk15) {
            case 0: {
                scorpionSetup = objAllocSetup(sizeof(Scorpion_Setup), OBJ_Scorpion);
                scorpionSetup->unk18 = mathRnd(-127, 126);
                ...
                objSetupObject((ObjSetup*)scorpionSetup, OBJINIT_STANDALONE | OBJINIT_FLAG4, self->mapID, -1, self->parent);
                break;
            }
            case 1: {
                scarabSetup = objAllocSetup(sizeof(Scarab_Setup), OBJ_Green_scarab);
                ...
                obj = objSetupObject((ObjSetup*)scarabSetup, OBJINIT_STANDALONE | OBJINIT_FLAG4, self->mapID, -1, self->parent);
                ...
                obj->velocity.x *= (1.0f - 0.0099999998f * mathRnd(0, 25));
                obj->velocity.z *= (1.0f - 0.0099999998f * mathRnd(0, 25));
                obj->velocity.y = 2.2f;
                srt.transl.x = 0.0f;
                srt.transl.y = 0.0f;
                srt.transl.z = 0.0f;
                ...
                srt.roll = 0;
                srt.pitch = 0;
                srt.yaw = mathRnd(-10000, 10000);
                mathRotateRPY(&srt, obj->velocity.f);
                temp = mathAtan2f(obj->velocity.x, -obj->velocity.z) & 0xFFFF & 0xFFFF & 0xFFFF;
            ...
            case 2: {
                scarabSetup = objAllocSetup(sizeof(Scarab_Setup), OBJ_Red_scarab);
                scarabSetup->unk18 = mathRnd(-127, 126);
                scarabSetup->base.x = self->srt.transl.x;
                scarabSetup->base.y = self->srt.transl.y;
                scarabSetup->base.z = self->srt.transl.z;
                scarabSetup->lifetime = 400;
            ...
            case 3: {
                scarabSetup = objAllocSetup(sizeof(Scarab_Setup), OBJ_Gold_scarab);
                scarabSetup->unk18 = mathRnd(-127, 126);
                scarabSetup->base.x = self->srt.transl.x;
                scarabSetup->base.y = self->srt.transl.y;
                scarabSetup->base.z = self->srt.transl.z;
            ...
            case 4: {
                scarabSetup = objAllocSetup(sizeof(Scarab_Setup), OBJ_Rain_scarab);
                scarabSetup->unk18 = mathRnd(-127, 126);
                scarabSetup->base.x = self->srt.transl.x;
                scarabSetup->base.y = self->srt.transl.y;
                scarabSetup->base.z = self->srt.transl.z;
            ...
            case 6: {
                if (objdata->unk15 == 5) {
                    foodPickupSetup = objAllocSetup(sizeof(Collectable_Setup), OBJ_meatPickup);
                } else {
                    foodPickupSetup = objAllocSetup(sizeof(Collectable_Setup), OBJ_applePickup);
                }
                ...
                obj = objSetupObject((ObjSetup*)foodPickupSetup, OBJINIT_STANDALONE | OBJINIT_FLAG4, self->mapID, -1, self->parent);
                obj->unkE0 = 21600;
                break;
            }
            case 7:
