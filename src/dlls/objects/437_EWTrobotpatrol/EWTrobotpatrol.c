...
static void EWTrobotpatrol_gunPrint(Object* self, ModelInstance* modelInst, Gfx** gdl, Mtx** mtxs, Vtx** vtxs, DLTri** pols);
static s32 EWTrobotpatrol_aimRaycast(Vec3f* barrelPos, Vec3f* aimPoint, Vec3f* fireAtPoint, Vec3f* a3, Object* target);
...
static void EWTrobotpatrol_curveMove(Object* self, EWTrobotpatrol_Data* objdata, EWTrobotpatrol_Body* body);
...
static void EWTrobotpatrol_fireGun(Object* self, EWTrobotpatrol_Gun* gun) {
    s32 angle;
    SRT startSRT;
    SRT endSRT;
...
static void EWTrobotpatrol_initBeam(Object* self, EWTrobotpatrol_Data* objdata, EWTrobotpatrol_Beam* beam) {
    ...
    beamSetup->quarterSize = 0x18; // @bug: should be divided by 4
    ...
    beamProjRot.yaw = self->def->pAttachPoints->rot.x;
    beamProjRot.pitch = self->def->pAttachPoints->rot.y;
    beamProjRot.roll = self->def->pAttachPoints->rot.z;
    beam->dir.x = 0.0f;
    beam->dir.y = 1.0f;
    beam->dir.z = 0.0f;
