...
void DIM_BossSpit_createLfxEmitter(Object* self) {
    ...
    objdata->lfxEmitter = objSetupObject(&lfxemitterSetup->base, OBJINIT_STANDALONE | OBJINIT_FLAG4, -1, -1, NULL);
}
...
void DIM_BossSpit_func_53C(Object* self) {
    ...
    if (objdata->unk0 == 1) {
        ...
        dll_amSfx->Play(self, SOUND_380_Scorching_Impact, MAX_VOLUME, NULL, NULL, 0, NULL);
    ...
    if (i >= 0) {
        ...
        if (lfxEmitter) {
            ...
            dll_newlfx->Func_BB8(lfxEmitterData->unk108->unk10, 0, i);
        }
    } else {
        lfxEmitter = objdata->lfxEmitter;
        if (lfxEmitter) {
