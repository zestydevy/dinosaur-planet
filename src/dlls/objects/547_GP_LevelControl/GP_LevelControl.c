...
void GP_LevelControl_create(Object *self, ObjCreateInfo *createInfo, s32 arg2) {
    ...
    case 1:
        set_gplay_bitstring(BIT_GP_Sharpclaw_Jetbike_Cutscene2, 1);
        break;
    case 3:
        set_gplay_bitstring(BIT_GP_Sharpclaw_Jetbike_Cutscene2, 1);
        break;
    ...
    set_gplay_bitstring(BIT_DFP_Place_Spellstone_One, 1);
    state->mapID = 0xFF;
}
...
void GP_LevelControl_update(Object *self) {
    ...
    if (!gDLL_7_Newday->vtbl->func8(&time) && !get_gplay_bitstring(BIT_643) && ((DLL_210_Player*)player->dll)->vtbl->func43(player) == 0) {
        ...
        if (state->heatCutsceneTimer > 600.0f) {
            ...
            if (!get_gplay_bitstring(BIT_GP_Shown_Heat_Cutscene)) {
                set_gplay_bitstring(BIT_GP_Shown_Heat_Cutscene, 1);
    ...
    if (gDLL_29_Gplay->vtbl->get_map_setup(MAP_GOLDEN_PLAINS) == 2) {
        ...
        if (obj && obj2 && vec3_distance_squared(&obj->positionMirror, &obj2->positionMirror) < 300.0f) {
            ...
            set_gplay_bitstring(BIT_GP_Sharpclaw_Jetbike_Cutscene1, 1);
            set_gplay_bitstring(BIT_GP_Sharpclaw_Jetbike_Cutscene2, 1);
        }
    }
}
...
void GP_LevelControl_func_4C8(Object *self) {
    ...
    if (mapSetup == 3) {
        set_gplay_bitstring(BIT_GP_Sharpclaw_Taken_Spellstone, 1);
    }
}
