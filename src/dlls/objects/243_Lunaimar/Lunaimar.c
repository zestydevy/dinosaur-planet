...
static void dll_243_func_EEC(Object *self, Baddie *baddie, ObjFSA_Data *fsa) {
    ...
    if (objdata->unk0 <= 0.0f) {
        gDLL_6_AMSFX->vtbl->play(self, _data_0[mathRnd(2, 3)], 0x30, &baddie->unk3A8, NULL, 0, NULL);
        objdata->unk0 = (f32) mathRnd(0xA0, 0x12C);
    }
    objdata->unk4 -= gUpdateRateF;
    if (objdata->unk4 <= 0.0f) {
        gDLL_6_AMSFX->vtbl->play(self, _data_8[mathRnd(0, 1)], MAX_VOLUME, NULL, NULL, 0, NULL);
...
static void dll_243_func_11C0(Object *self, Baddie *baddie, ObjFSA_Data *fsa) {
    ...
    if (fsa->unk308 & 1) {
        fsa->unk308 &= ~1;
        gDLL_6_AMSFX->vtbl->play(self, _data_0[3], MAX_VOLUME, &baddie->unk3A8, NULL, 0, NULL);
    }
    if (fsa->unk308 & 0x80) {
        objdata->unk10 = mathRnd(0, 2);
        fsa->unk308 &= ~0x80;
        gDLL_6_AMSFX->vtbl->play(self, _data_0[objdata->unk10], MAX_VOLUME, &baddie->unk3A8, NULL, 0, NULL);
    ...
    if (fsa->unk308 & 0x200) {
        fsa->unk308 &= ~0x200;
        gDLL_6_AMSFX->vtbl->play(self, SOUND_AA4_Hock_Up, MAX_VOLUME, NULL, NULL, 0, NULL);
    }
    if (fsa->unk308 & 0x400) {
        objdata->unk10 = 3;
        i = 10;
...
static void dll_243_func_1614(Object *self, Lunaimar_ActualData *objdata) {
    ...
    objdata->unkC = 255.0f;
    gDLL_6_AMSFX->vtbl->play(self, SOUND_AB4_Magic_Barrier_Deflect, MAX_VOLUME, NULL, NULL, 0, NULL);
    for (i = 20; i > 0; i--) {
