...
int CCspellpage_anim_callback(Object *self, Object *animObj, AnimObj_Data *animObjData, s8 arg3) {
    ...
    //Show the info scroll in the top-left of screen
    dShowInfoScrollTimer -= gUpdateRateF;
    if (dShowInfoScrollTimer > 0.0f) {
        gDLL_1_cmdmenu->vtbl->auto_show_info_scroll(GAMETEXT_UI_1D_Forcefield_Spell, 80, 60);
    }
