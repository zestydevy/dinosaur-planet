...
/*0x0*/ static u16 _data_0[3] = {
    FLAG_Swapstone_Circle_Hit_Village_Tree_One,
    FLAG_Swapstone_Circle_Hit_Village_Tree_Two,
    FLAG_Swapstone_Circle_Hit_Village_Tree_Three
};
...
void SC_levelcontrol_create(Object *self, ObjCreateInfo *createInfo, s32 arg2) {
    ...
    state->unk2 = 0;
    state->index1 = get_gplay_bitstring(FLAG_2BA);
...
void SC_levelcontrol_update(Object *self) {
    ...
    player = get_player();
    sp2F = 0;
    if (get_gplay_bitstring(FLAG_4BD)) {
        set_gplay_bitstring(FLAG_4BD, 0);
        sp2F = 1;
        state->index1 = 15;
    }
    ...
    if (get_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_One)) {
        set_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_One, 0);
        if (_data_0[state->treesHit] == FLAG_Swapstone_Circle_Hit_Village_Tree_One) {
            state->treesHit++;
        } else {
            state->treesHit = 0;
        }
    } else if (get_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_Two)) {
        set_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_Two, 0);
        if (_data_0[state->treesHit] == FLAG_Swapstone_Circle_Hit_Village_Tree_Two) {
            state->treesHit++;
        } else {
            state->treesHit = 0;
        }
    } else if (get_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_Three)) {
        set_gplay_bitstring(FLAG_Swapstone_Circle_Hit_Village_Tree_Three, 0);
        if (_data_0[state->treesHit] == FLAG_Swapstone_Circle_Hit_Village_Tree_Three) {
            state->treesHit++;
        } else {
            state->treesHit = 0;
        }
    ...
    if (state->treesHit >= 3) {
        set_gplay_bitstring(FLAG_Swapstone_Circle_Village_Opened, 1);
        state->treesHit = 0;
    }
    ...
    if (state->unk2 != 0) {
        if (state->unk2 == 5) {
            if (func_8000FB1C() != 0) {
                set_gplay_bitstring(FLAG_7CF, 1);
            }
        } else if ((get_gplay_bitstring(FLAG_Dont_Repeat_Swapstone_Intro)) && (func_8000FB1C() != 0)) {
            set_gplay_bitstring(FLAG_87, 0);
...
void SC_levelcontrol_destroy(Object *self, s32 arg1) {
    ...
    set_gplay_bitstring(FLAG_2BA, state->index1);
    func_8000FA2C();
...
void SC_levelcontrol_func_660(Object *self, u8 arg1) {
    ...
    if (state->unk2 == 1) {
        set_gplay_bitstring(FLAG_87, 1);
        set_gplay_bitstring(FLAG_Swapstone_Circle_Platform_Rises_Totem_Challenge_Begins, 1);
        gDLL_5_AMSEQ2->vtbl->func0(self, 0xEC, 0, 0, 0);
...
void SC_levelcontrol_func_8B4(Object *self) {
    ...
    if (mapSetup < 3) {
        if (get_gplay_bitstring(FLAG_Cape_Claw_Used_Cell_Door_Key)) {
    ...
    case 3:
        ...
        break;
    case 8:
        if (get_gplay_bitstring(FLAG_Spirit_Release_Sabre)) {
            gDLL_29_Gplay->vtbl->set_map_setup(MAP_SWAPSTONE_CIRCLE, 9);
        }
        break;
    case 9:
        if (get_gplay_bitstring(FLAG_Spellstone_Activated_Cloudrunner_Fortress)) {
            gDLL_29_Gplay->vtbl->set_map_setup(MAP_SWAPSTONE_CIRCLE, 5);
        }
        break;
    case 5:
        if (get_gplay_bitstring(FLAG_317)) {
    ...
    case 1:
        ...
        break;
    case 6:
        set_gplay_bitstring(FLAG_Swapstone_Circle_Play_Krystal_Sees_Tribe_Dino, 0);
        set_gplay_bitstring(FLAG_Swapstone_Circle_Played_Krystal_Sees_Tribe_Dino, 0);
        break;
    }
}
...
void SC_levelcontrol_func_11FC(Object *self) {
    if (!get_gplay_bitstring(FLAG_Swapstone_Circle_Ambushed) &&
        get_gplay_bitstring(FLAG_Swapstone_Circle_Encountered_Baby_Lightfoot)) {
        set_gplay_bitstring(FLAG_Swapstone_Circle_Ambushed, 1);
