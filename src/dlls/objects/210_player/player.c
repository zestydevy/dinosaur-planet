...
s32 dll_210_func_125BC(Object *self, ObjFSA_Data *fsa, f32 updateRate) {
    f32 temp_fs0;
    f32 temp_fs1;
    f32 f2;
    f32 f0;
    ...
    if (fsa->enteredAnimState != 0) {
        ...
        for (i = 0; i < 3; i++) {
            temp_fs0 = ((f32) rand_next(-0x32, 0x32) / 10.0f) + self->srt.transl.x;
            temp_fs1 = ((f32) rand_next(-0x32, 0x32) / 10.0f) + self->srt.transl.z;
