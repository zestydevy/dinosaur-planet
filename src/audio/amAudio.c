...
void func_80069378(void *oscState);
...
ALMicroTime updateOsc(void *oscState, f32 *updateVal) {
	...
	case 0x80:
		...
		sp2c = (f32)state->unk24 / (f32)state->unk22;
		sp2c = sinf(sp2c * DEG_TO_RAD(360)) * state->data.b.unk28;
...
ALMicroTime initOscMain(void **oscState, f32 *initVal, u8 oscType, u8 arg3, u8 oscDepth, u8 arg5, u8 arg6) {
	...
	if (freeOscStateList == NULL) {
        osSyncPrintf("amAudio: WARNING no free oscstates.\n");
		return 0;
	}
	...
	if (arg6 == 0) {
		...
		state->unk18 = 0;
		state->unk1c = 1.0f / ((f32)D_800967A0[arg6] / AL_USEC_PER_FRAME);
	}

	state->type = oscType;
	state->unk14 = 0;
	state->unk16 = 1000000.0f / (f32)D_8009699C[arg3] / AL_USEC_PER_FRAME;
	...
	case 5:
		...
		state->curCount = 500000.0f / (f32)D_8009699C[arg3];
		break;
	case 6:
	case 8:
	case 11:
	...
	case 10:
		...
		break;
	default:
        osSyncPrintf("amAudio: unsupported vibrato/tremolo type,%d\n", state->type);
		return 0;
	}
...
ALMicroTime updateOscMain(oscData *statePtr, f32 *updateVal) {
	...
	case 8:
	case 9:
		sp20 = func_80068924(sp20 * 65536.0f) * sp24 + statePtr->unk10;
		break;
	case 10:
	case 11:
		sp20 *= sp24;
...
void func_8006939C(ALSeqpConfig *config, s32 count) {
    oscData *item;
	s32 i;

    oscStates = (oscData*)alHeapAlloc(config->heap, count, sizeof(oscData));
    freeOscStateList = oscStates;
    freeOscStateList = oscStates; // hmm
